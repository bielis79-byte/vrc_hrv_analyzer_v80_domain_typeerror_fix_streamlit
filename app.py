...
st.set_page_config(page_title="VRC / HRV RRi Analyzer Pro v8.0", layout="wide")
...
COLOR_MAP = {
    ...
    "MSE corto plazo (1-5)": "#1f77b4",
    "MSE medio plazo (6-12)": "#ff7f0e",
    "MSE largo plazo (13-20)": "#2ca02c",
}

MSE_COLOR_MAP = {}
for _i in range(1, 21):
    if _i <= 5:
        MSE_COLOR_MAP[f"MSE{_i}"] = "#1f77b4"
    elif _i <= 12:
        MSE_COLOR_MAP[f"MSE{_i}"] = "#ff7f0e"
    else:
        MSE_COLOR_MAP[f"MSE{_i}"] = "#2ca02c"


def metric_color(metric):
    """Color fijo para cada parámetro en toda la app."""
    return COLOR_MAP.get(metric, MSE_COLOR_MAP.get(metric, "#7f7f7f"))
...
def mse_group_label(scale):
    if scale <= 5:
        return "MSE corto plazo (1-5)"
    if scale <= 12:
        return "MSE medio plazo (6-12)"
    return "MSE largo plazo (13-20)"


def mse_group_color(scale):
    return COLOR_MAP[mse_group_label(scale)]
...
def classify_hvg_graph_type(metrics):
    ...
    No es un diagnóstico matemático cerrado; resume rasgos de:
    - escala libre / jerárquico: hubs marcados, distribución de grados heterogénea.
    - small-world funcional: clustering relativamente alto y camino medio/diámetro contenido.
    - cadena / lineal: pocas conexiones, bajo grado medio, diámetro alto.
    - regular / homogéneo: pocos hubs, grado máximo bajo-moderado, clustering bajo-moderado.
    - complejo mixto: combinación de propiedades.
    """
    try:
    ...
    if np.isfinite(lam):
        # Lambda baja/moderada suele indicar cola más pesada; lambda muy alta, red más homogénea.
        if lam < 0.45:
            scale_free_score += 20
        elif lam < 0.75:
            scale_free_score += 12
...
def domain_values(metrics_df, method="median"):
    ...
    v8.0:
    - evita errores cuando existen columnas no numéricas, como HVG_graph_type.
    - sólo usa valores convertibles a número.
    """
    ...
    for ph in [p for p in PHASES if p in metrics_df.index]:
        ...
        for dom, vars_ in DOMAIN_GROUPS.items():
            ...
            for v in vars_:
                if v not in metrics_df.columns or v not in base.index:
                    continue
...
def domains_figure(metrics_df, method="median", title="Dominios Amplitud / Vagal / Complejidad / Recurrencia"):
    """
    Dominios con curvas suavizadas.
    Basal = 100%.
    """
    ...
    for col in dom.columns:
        ...
        color = metric_color(col) if "metric_color" in globals() else None

        xs, ys = _smooth_line_xy(y) if "_smooth_line_xy" in globals() else (x_base, y)
        ...
        fig.add_trace(go.Scatter(
            ...
            showlegend=False,
            hovertemplate=f"{col}<br>Fase: %{{customdata}}<br>Valor: %{{y:.1f}}%<extra></extra>",
            customdata=phases,
        ))
...
def mse_figure(metrics_df, title="MSE 1-20"):
    """MSE 1-20 en una única gráfica: barras agrupadas por fase + líneas por escala. Leyenda opción B."""
    fig = go.Figure()
    ...
    for col in mse_cols:
        try:
            scale = int(col.replace("MSE", ""))
        except Exception:
            scale = 1
        group_label = mse_group_label(scale)
        color = mse_group_color(scale)
        show_group = group_label not in shown_groups
        shown_groups.add(group_label)

        y = [metrics_df.loc[ph, col] if ph in metrics_df.index else np.nan for ph in phases]
        fig.add_trace(go.Bar(x=phases, y=y, name=group_label, opacity=0.32,
                             marker=dict(color=color),
                             legendgroup=group_label,
                             showlegend=show_group,
...
def hvg_explanation_table(metrics_df):
    """
    Tabla enriquecida para Métricas HVG / grafos.
    """
    ...
    cols = [c for c in hvg_cols if c in metrics_df.columns]
    if not cols:
        return pd.DataFrame()

    df = metrics_df[cols].copy()
    df.insert(0, "Fase", df.index)

    explanations = {
        "HVG_graph_type": "Tipo de organización topológica dominante.",
        "HVG_graph_score_scale_free": "Puntuación 0-100 de rasgos tipo libre de escala / hubs.",
        "HVG_graph_score_small_world": "Puntuación 0-100 de rasgos small-world: clustering + caminos cortos.",
        "HVG_graph_score_chain": "Puntuación 0-100 de rasgos lineales/cadena: pocas conexiones y diámetro alto.",
        "HVG_nodes": "Número de nodos analizados.",
        "HVG_edges": "Número de conexiones visibles entre nodos.",
        "HVG_degree_mean": "Grado medio: conexiones promedio por nodo.",
        "HVG_degree_max": "Grado máximo: nodo más conectado.",
        "HVG_hubs_p90": "Número de nodos con grado alto, por encima del percentil 90.",
        "HVG_clustering": "Agrupamiento local de la red.",
        "HVG_lambda": "Pendiente/exponente aproximado de la distribución de grados.",
        "HVG_path_length": "Camino medio: pasos promedio entre nodos.",
...
def dashboard_bar_smooth(long_df, phases, params):
    """
    Dashboard evolutivo robusto:
    - evita ValueError de Plotly por horizontal_spacing excesivo,
    - usa como máximo 2 columnas,
    - altura adaptativa para que no se aplasten los paneles,
    - colores fijos cuando el parámetro existe en COLOR_MAP.
    """
    params = [p for p in params if p in long_df.columns]
    ...
    # Plotly exige horizontal_spacing <= 1/(cols-1). Con cols=2 no hay problema,
    # pero lo dejamos dinámico y conservador.
    h_spacing = 0.08 if cols == 2 else 0.0
    ...
    for idx, param in enumerate(params):
        ...
        if one_phase:
            ...
            bar_kwargs = dict(x=x_labels, y=y_vals, name=f"{param} · barras", opacity=0.66, showlegend=False)
            if color:
                bar_kwargs["marker"] = dict(color=color)

            fig.add_trace(go.Bar(**bar_kwargs), row=r, col=c)

            xs, ys = _smooth_line_xy(y_vals) if "_smooth_line_xy" in globals() else (list(range(len(y_vals))), y_vals)
            # map numeric xs to labels if smoothed: use numeric axis for smooth lines in this case
            fig.add_trace(
                go.Scatter(
                    x=list(range(len(x_labels))) if len(xs) == len(y_vals) else xs,
                    y=ys,
                    mode="lines+markers" if len(xs) == len(y_vals) else "lines",
            ...
            bar_kwargs = dict(
                x=x_labels, y=y_vals, name=f"{param} · barras", opacity=0.66,
...
def _single_record_report(record_name, metrics_df, windows):
    ...
    metrics = ["MeanHR","SDNN","RMSSD","pNN50","SD1","SD2","VLF","LF","HF","TOTAL","DFA_alpha1","DFA_alpha2","ApEn","SampEn","REC","DET","Lmean","Lmax","ShanEn",
               "HVG_graph_type","HVG_graph_score_scale_free","HVG_graph_score_small_world","HVG_graph_score_chain","HVG_edges","HVG_degree_mean","HVG_degree_max","HVG_hubs_p90","HVG_clustering","HVG_lambda","HVG_path_length","HVG_diameter"] + [f"MSE{i}" for i in range(1,21)]
...
def _smooth_line_xy(y_values, smooth_points=260):
    """
    Línea de tendencia visualmente suavizada.
    - Con 2 puntos: línea recta.
    - Con 3 puntos: ajuste cuadrático suave.
    - Con 4 o más puntos: CubicSpline natural.
    """
...
def _add_custom_panel_legend(fig, panel_id, labels):
    """
    Leyenda manual con anotaciones.
    Evita usar legend2/legend3, que falla en algunas versiones de Plotly/Streamlit Cloud.
    """
    positions = {
        1: (0.455, 0.965),
        2: (0.985, 0.965),
    ...
    x, y = positions.get(panel_id, (0.98, 0.98))

    for i, label in enumerate(labels):
        raw = {"DFA α1": "DFA_alpha1", "DFA α2": "DFA_alpha2"}.get(label, label)
        color = metric_color(raw) if raw in COLOR_MAP or raw in MSE_COLOR_MAP else COLOR_MAP.get(label, "#7f7f7f")
        fig.add_annotation(
            x=x,
            y=y - i * 0.022,
            xref="paper",
...
def _add_bars_and_smooth_lines(
    fig,
    metrics_df,
    row,
    col,
    ...
    panel_id,
    yaxis_title="Valor",
    secondary_y_metric=None,
    secondary_y_title=None,
    mse_grouped_legend=False,
):
    """
    Columnas verticales + líneas suavizadas.
    La leyenda se dibuja manualmente fuera del panel con anotaciones.
    """
    phases = [p for p in PHASES if p in metrics_df.index]
    if not phases:
        return []
    ...
    for i, m in enumerate(present):
        ...
        if m.startswith("MSE"):
            try:
                scale = int(m.replace("MSE", ""))
            except Exception:
                scale = 1
            color = mse_group_color(scale)
            group_label = mse_group_label(scale)
            if mse_grouped_legend and group_label not in shown_mse_groups:
                legend_labels.append(group_label)
                shown_mse_groups.add(group_label)
        else:
            color = metric_color(m)
            legend_labels.append(metric_label(m))
        ...
        fig.add_trace(
            go.Bar(
                ...
                marker=dict(color=color),
                showlegend=False,
                hovertemplate=f"{metric_label(m)}<br>Fase: %{{customdata}}<br>Valor: %{{y:.3f}}<extra></extra>",
                customdata=phases,
            ),
            row=row,
            col=col,
        ...
        fig.add_trace(
            go.Scatter(
                ...
                y=ys,
                mode="lines",
                name=f"{metric_label(m)} tendencia",
...
def hrv_phase_summary_figure(metrics_df):
    """
    Resumen HRV 6 paneles:
    columnas verticales + líneas suavizadas + leyendas laterales manuales.
    Compatible con Plotly/Streamlit Cloud porque no usa legend2/legend3.
    """
    if metrics_df is None or metrics_df.empty:
        fig = go.Figure()
    ...
    legends[1] = _add_bars_and_smooth_lines(
        fig, metrics_df, 1, 1,
        ["RMSSD", "SDNN", "pNN50"],
        panel_id=1,
        yaxis_title="ms",
    ...
    legends[2] = _add_bars_and_smooth_lines(
        fig, metrics_df, 1, 2,
        ["VLF", "LF", "HF", "TOTAL"],
        panel_id=2,
        yaxis_title="ms²",
    ...
    legends[3] = _add_bars_and_smooth_lines(
        fig, metrics_df, 2, 1,
        ["SD1", "SD2"],
        panel_id=3,
        yaxis_title="ms",
    ...
    legends[4] = _add_bars_and_smooth_lines(
        fig, metrics_df, 2, 2,
        complexity_vars,
        panel_id=4,
        yaxis_title="Valor",
    ...
    legends[5] = _add_bars_and_smooth_lines(
        fig, metrics_df, 3, 1,
        recurrence_vars,
        panel_id=5,
        yaxis_title="Valor",
    ...
    legends[6] = _add_bars_and_smooth_lines(
        fig, metrics_df, 3, 2,
        mse_vars,
        panel_id=6,
        yaxis_title="Valor",
        mse_grouped_legend=True,
    )

    for pid, labels in legends.items():
        _add_custom_panel_legend(fig, pid, labels)
    ...
    fig.update_layout(
        ...
        bargap=0.22,
        bargroupgap=0.02,
        margin=dict(l=60, r=260, t=100, b=70),
        showlegend=False,
    )
...
st.title("VRC / HRV RRi Analyzer Pro v8.0")
...
with tab5:
    ...
    if len(records) < 1:
        ...
        if not available_phases_pg:
            ...
            hvg_long = hvg_explanation_table(records_results.get(selected_record, pd.DataFrame()))
            if not hvg_long.empty:
                st.caption("Clasificación orientativa del tipo de grafo y explicación de cada métrica.")
                st.dataframe(hvg_long, use_container_width=True)
